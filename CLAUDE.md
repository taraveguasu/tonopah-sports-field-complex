...
| Voice profile | ✅ **Re-derived from the PM's own writing (08.08.26)** — `.claude/skills/attachment-a-generator/references/voice-profile.md`, measured from `01-index/voice-corpus.jsonl` by `scripts/build_voice_corpus.py`, enforced by `scripts/voice_check.py`. Corpus: **108 authored lines the PM wrote**, from 3 executed DMV Silverado Ranch exhibits across 3 trades (ADI doors/frames/hardware, Adams & Smith structural/ornamental metals, Anning-Johnson metal studs/drywall) in `00-source-docs/voice-corpus/mine/`; 162 `core` lines (another CORE PM's masonry exhibit + 3 blank templates); 138 `claude` draft lines. **Only the SCOPE OF WORK section of the PM's exhibits is his** — documents/provisions/options/exclusions are the contracts department's and are tagged `core` even inside his files (`SELF_SECTIONS`). Key measured findings: median item **12 words** (drafts run 24–27); **`Includes …` opens 28%** of his items vs 1% elsewhere; **`Supply and install`** 18% vs 1% (`Provide and install` is the *template's* verb, 1 use in 108); **zero** spec-section citations in scope items — he cites details/sheets (`per 5/S5.44`); bare numerals, not `thirty-four (34)`; and he **uses CMU/MEP/FRP/HSS freely**. D1 and D3 closed; **D2 (exclusion style) stays open and this corpus cannot close it** — he did not write those sections |
