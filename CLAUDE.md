...
| Voice profile | ⚠️ Built but **it is a HOUSE profile, not the PM's voice** (08.08.26) — `.claude/skills/attachment-a-generator/references/voice-profile.md`, measured from `01-index/voice-corpus.jsonl` by `scripts/build_voice_corpus.py`, enforced by `scripts/voice_check.py`. **Nothing staged in this repo was authored by the PM.** Corpus is 49 authored lines from ONE executed exhibit written by a *different* CORE PM (PTS Masonry, TRUSD Highlands HS, 22-01-005 — a CORE West job, found inside the Bluebeam process example) plus 41 lines from 3 blank CORE templates. Records carry an `author` field (`self` / `core` / `claude`); `self` count is currently **0**. 12 rules; 3 open decisions (D1 `Provide and install` vs `Supply and install`, D2 exclusion style, D3 standing operational obligations) — all three are personal-practice questions the house corpus cannot settle. **PM is pulling executed exhibits he wrote → they go in `00-source-docs/voice-corpus/mine/` (the subfolder IS the authorship signal), then re-run the builder.** That emits a "Where yours diverges from CORE's other authors" table, which is the real personal profile, and the rules get re-derived against `self` evidence rather than assumed |
