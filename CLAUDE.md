...
| Voice profile | ✅ **Derived from the PM's own writing (08.08.26)** — `.claude/skills/attachment-a-generator/references/voice-profile.md`, measured from `01-index/voice-corpus.jsonl` by `scripts/build_voice_corpus.py`, enforced by `scripts/voice_check.py`. Corpus: **411 authored lines the PM wrote, 386 scope items**, from **8 executed DMV Silverado Ranch exhibits (23-01-011) across 8 trades** — ADI doors/frames/hardware, Adams & Smith structural/ornamental metals, Anning-Johnson studs/drywall, Great Salt Lake Electric, Rosendin low voltage, Southland building & lighting controls, Performance Contracting, XL Concrete Masonry. Source: the PM's OneDrive `Documents/DMV Database/Final Att A/` (~30 exhibits total; the rest are unpulled). **Only the SCOPE OF WORK section is his** — documents/provisions/options/exclusions are the contracts department's and are tagged `core` even inside his files (`SELF_SECTIONS`). Measured: median item **14 words** (drafts run 24–27, p99 50, error threshold 55); **`Includes …` opens 27%** (104/386) vs <1% elsewhere; **`Supply and install` 14.2%** vs `Provide and install` 4/386; **`Coordinate` opens 10.4%**, 14% carry a coordination sentence; **spec-section citations 2/386** — he cites details/sheets (`per 5/S5.44`); bare numerals, never `thirty-four (34)`; **uses CMU/MEP/FRP/HSS/BMS/AHU freely**. Group-header structure never varies (15/25 verbatim, 6 casing-only). D1 and D3 closed; **D2 (exclusion style) stays open — he did not write those sections**. OneDrive exhibits come back as text, not PDF bytes: `scripts/ingest_packet_text.py` finds the exhibit inside the packet by its page footers |
