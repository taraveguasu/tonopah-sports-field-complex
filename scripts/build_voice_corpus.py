#!/usr/bin/env python3
"""
Build the voice corpus: every line of Attachment A scope language CORE actually
authored, separated from the template boilerplate it sits in.

A voice profile is only as good as the corpus under it, and the corpus problem
here is that most of an Attachment A is not written by anyone -- it is the
template's fixed contract prose, identical on every exhibit. Counting it teaches
nothing except that the template exists. So every line collected here is tagged
`authored` or `boilerplate`, and the statistics only ever run on the authored
half.

Three sources, in descending order of authority:

  executed   A signed/approved Attachment A from a completed job. This is ground
             truth -- prose that survived a Review Session, a Final Session and
             a subcontractor's own counsel. Weighted highest.
  template   The authored example bullets CORE leaves inside its own .docx
             templates (the highlighted runs). Written by CORE to show the
             house style, so they are intentional, but they are illustrative
             rather than executed.
  draft      This project's own content records. Included so drift between what
             we are generating now and what CORE actually signs is measurable
             rather than a matter of opinion. Never used to define a rule --
             only to be checked against one.

Drop additional executed exhibits (.pdf or .docx) into
00-source-docs/voice-corpus/ and re-run; they are picked up automatically and
are the single highest-value thing that can be added to this pipeline.
...
FURNITURE = re.compile(
    ...
    r"subcontractor:|supplier:|password:|invite to be sent)", re.I
)
...
def parse_exhibit(lines, source_id, kind):
...
def docx_highlighted(path, source_id):
...
def measure(records):
    ...
    for kind in ("executed", "template", "draft"):
        rows = [r for r in records if r["source_kind"] == kind and r["authored"]]
        groups[kind] = rows
    ...
    lines.append("")
    lines.append("| source | authored lines | boilerplate lines | documents |")
    lines.append("|---|---:|---:|---:|")
    for kind, rows in groups.items():
        boiler = sum(1 for r in records
                     if r["source_kind"] == kind and not r["authored"])
        docs = len({r["source_id"] for r in records if r["source_kind"] == kind})
        lines.append(f"| {kind} | {len(rows)} | {boiler} | {docs} |")
    lines.append("")

    lines.append("## Probe hits (share of that source's authored lines)")
    lines.append("")
    lines.append("| probe | executed | template | draft |")
    lines.append("|---|---:|---:|---:|")
    ...
    lines.append("")
    lines.append("| source | items | median words | median sentences |")
    lines.append("|---|---:|---:|---:|")
    for kind in groups:
        rows = [r for r in scope if r["source_kind"] == kind]
        if not rows:
        ...
        lines.append(f"| {kind} | {len(rows)} | {words[len(words)//2]} | "
                     f"{sents[len(sents)//2]} |")
    lines.append("")

    openers = collections.Counter()
    for r in scope:
        if r["source_kind"] in ("executed", "template"):
            openers[" ".join(r["text"].split()[:2])] += 1
    lines.append("## Most common openers in executed + template scope items")
    lines.append("")
...
def main():
    ...
    pdfs = [p for p in SEEDS if p.exists()]
    if DROP.exists():
        pdfs += sorted(DROP.glob("*.pdf"))
    for p in pdfs:
        recs = parse_exhibit(pdf_lines(p), p.stem, "executed")
        records += recs
        print(f"executed  {p.name}: {len(recs)} lines")
    ...
    if DROP.exists():
        docx += sorted(DROP.glob("*.docx"))
    for p in docx:
        recs = docx_highlighted(p, p.stem)
        records += recs
        print(f"template  {p.name}: {len(recs)} lines")

    for p in sorted(CONTENT.glob("*.json")):
        recs = draft_records(p)
        records += recs
        print(f"draft     {p.stem}: {len(recs)} lines")
